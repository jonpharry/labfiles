...
var samSchema  = "urn:ietf:params:scim:schemas:extension:isam:1.0:User";

var scimSkeleton = {
  "schemas":[
    coreSchema,
    enterpriseSchema, samSchema
  ]
};
...
//  Parameter         [ key1, key2 ]                         Required Default Value
var attrToScim = [
  [ "username",       ["userName"],                           true,  null    ],
  [ "firstName",      ["displayName"],                        true,  null    ],
  [ "firstName",      ["name", "givenName"],                  true,  null    ],
  [ "surname",        ["name", "familyName"],                 true,  null    ],
  [ "title",          ["title"],                              false,  null    ],
  [ "addressType",    ["addresses", "type"],                  false,  "work"  ],
  [ "address",        ["addresses", "formatted"],             false,  null    ],
  [ "streetAddress",  ["addresses", "streetAddress"],         false,  null    ],
  [ "postcode",       ["addresses", "postalCode"],            false,  null    ],
  [ "emailType",      ["emails", "type"],                     true,  "work"  ],
  [ "email",          ["emails", "value"],                    true,  null    ],
  [ "password",       ["password"],                           true,  null    ],
  [ "language",       ["preferredLanguage"],                  false, "en-us" ],
  [ "employeeNumber", [enterpriseSchema, "employeeNumber"],   true,  null    ],
  [ "department",     [enterpriseSchema, "department"],       false,  null    ],
  [ "organization",   [enterpriseSchema, "organization"],     false,  null    ],
  [ "username",       [samSchema, "identity"],                true,  null    ],
  [ "accountValid",   [samSchema, "accountValid"],            true,  "true"  ]

];

// Convenience enum-like structure for accessing attrToScim
var attrToScimMap = {
  ATTR: 0, KEYS: 1, REQ: 2, DEF: 3
}
 ...
 * object.
  *
 * @param keys[] The array of hash keys.
 * @param value  The value to set.
 * @param hash{} The associative array to modify.
 */
function setInHash(keys, value, hash) {
  if (hash == null) return;
  var hashOrig = hash;
  var key = 0;
...
function createScimJson() {
  ...
  for (var attr in attrToScim) {
    ...
    if (val == "true" || val == "false")
       setInHash(a[attrToScimMap.KEYS], val == "true", scim);
    else
       setInHash(a[attrToScimMap.KEYS], ""+val, scim)
  }
